...
//! `<input>` / anchor-download handle it; on desktop these commands fill it with
//! the real Finder open/save panels. The dialog runs in the Rust process and we
//! read/write the chosen path here, so the webview never needs filesystem scope
//! — the user's explicit pick through the OS panel is the only grant.
...
/// Read the given paths into `PickedFile`s. Used for OS drag-drop, where Tauri
/// hands the frontend real paths (the user's drop is the grant) that still need
/// their bytes read in the Rust process. A path that isn't a readable file — a
/// dropped folder, or one that vanished between the drop and the read — is
/// skipped rather than failing the whole drop, so the other dropped files still
/// come through. The frontend's own allowlist then filters by extension.
///
...
/// its bytes are read — so an oversized drop never gets pulled into memory and
/// shipped over IPC only to be rejected. `None` reads every file regardless.
pub fn read_files(paths: Vec<PathBuf>, max_bytes: Option<u64>) -> Vec<PickedFile> {
    paths
        .iter()
