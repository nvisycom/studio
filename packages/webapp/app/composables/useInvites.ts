...
export function useInvites() {
	...
	const invitesQuery = useQuery({
		key: () => ["invites", currentWorkspaceId.value],
		query: async () => {
			const client = $nvisyClient.value;
			const workspaceId = currentWorkspaceId.value;
			if (!client) throw new Error("Not authenticated");
			if (!workspaceId) throw new Error("No workspace selected");
			return await client.invites.list(workspaceId);
		},
	...
	const sendInviteMutation = useMutation({
		mutation: async (invite: CreateInvite) => {
			...
			if (!client) throw new Error("Not authenticated");
			if (!workspaceId) throw new Error("No workspace selected");
			return await client.invites.send(workspaceId, invite);
		},
		onSuccess() {
	...
	const cancelInviteMutation = useMutation({
		mutation: async (inviteId: string) => {
			const client = $nvisyClient.value;
			if (!client) throw new Error("Not authenticated");
			await client.invites.cancel(inviteId);
		},
		onSuccess() {
			invitesQuery.refresh();
		},
	...
	const generateCodeMutation = useMutation({
		mutation: async (options: GenerateInviteCode) => {
			...
			if (!client) throw new Error("Not authenticated");
			if (!workspaceId) throw new Error("No workspace selected");
			return await client.invites.generateCode(workspaceId, options);
		},
	});
