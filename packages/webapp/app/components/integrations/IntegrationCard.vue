...
<template>
  <Card class="overflow-hidden border-neutral-200 dark:border-neutral-800 flex flex-col">
    <CardHeader>
      <div class="flex items-start justify-between">
        <div class="flex items-center gap-3">
          <div :class="['w-12 h-12 rounded-lg flex items-center justify-center', integration.color]">
            <component :is="integration.icon" :size="24" class="text-white" />
          </div>
          <div>
            <CardTitle class="text-lg">{{ integration.name }}</CardTitle>
            <p class="text-xs text-neutral-600 dark:text-neutral-400 mt-1">{{ integration.category }}</p>
          </div>
        </div>
        <Badge
          :variant="integration.status === 'available' ? 'secondary' : 'outline'"
          :class="integration.status === 'available'
            ? 'bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300'
            : 'bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400'"
        >
          {{ integration.status === 'available' ? t('integrations.status.available') : t('integrations.status.comingSoon') }}
        </Badge>
      </div>
    </CardHeader>
    ...
    <CardFooter class="pt-4">
      ...
      <Button
        ...
        class="w-full"
      >
        <a :href="integration.externalUrl" target="_blank" rel="noopener noreferrer" class="flex items-center justify-center gap-2">
